...
describe('Hero Service', () => {
  ...
  describe('getHero', () => {
    it('should return a single mock hero', () => {
      ...
      heroService
        .getHero(this.mockHero.id)
        .subscribe((response) => expect(response).toEqual(this.mockHero), fail);
      // Receive GET request
      const req = httpTestingController.expectOne(
        `${heroService.heroesUrl}/${this.mockHero.id}`
      );
      expect(req.request.method).toEqual('GET');
      // Respond with the mock heroes
      ...
      expect(heroService.log).toHaveBeenCalledTimes(1);
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: fetched hero id=${this.mockHero.id}`
      );
    });
    ...
    it('should fail gracefully on error', () => {
      ...
      expect(heroService.log).toHaveBeenCalledTimes(1);
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: getHero id=${this.mockHero.id} failed: Http failure response for ${heroService.heroesUrl}/${this.mockHero.id}: 404 Bad Request`
      );
    });
  });
  ...
  describe('getHeroNo404', () => {
    it('should return a single mock hero', () => {
      ...
      // Receive GET request
      const req = httpTestingController.expectOne(
        `${heroService.heroesUrl}/?id=${this.mockHero.id}`
      );
      expect(req.request.method).toEqual('GET');
      // Respond with the mock heroes
      req.flush(this.mockHeroes);
    ...
    it('should fail gracefully with undefined when id not found', () => {
      ...
      expect(heroService.log).toHaveBeenCalledTimes(1);
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: did not find hero id=${this.mockHero.id}`
      );
    });
    ...
    it('should fail gracefully on error', () => {
      ...
      const msg = 'Deliberate 404';
      heroService
        .getHeroNo404(this.mockHero.id)
        .subscribe((heroes) => expect(heroes).toBeUndefined(), fail);
      ...
      expect(heroService.log).toHaveBeenCalledTimes(1);
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: getHero id=${this.mockHero.id} failed: Http failure response for ${heroService.heroesUrl}/?id=${this.mockHero.id}: 404 Bad Request`
      );
    });
  });
  ...
  describe('addHero', () => {
    it('should add a single Hero', () => {
      ...
      expect(heroService.log).toHaveBeenCalledTimes(1);
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: added hero w/ id=${this.mockHero.id}`
      );
    });
  ...
  describe('updateHero', () => {
    it('should update hero', () => {
      ...
      expect(heroService.log).toHaveBeenCalledTimes(1);
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: updateHero failed: Http failure response for ${heroService.heroesUrl}: 404 Bad Request`
      );
    });
    ...
    it('should fail gracefully on error', () => {
      ...
      expect(heroService.log).toHaveBeenCalledTimes(1);
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: updated hero id=${this.mockHero.id}`
      );
    });
  });
  ...
  describe('deleteHero', () => {
    it('should delete hero using id', () => {
      ...
      heroService
        .deleteHero(this.mockId)
        .subscribe((response) => expect(response).toEqual(this.mockId), fail);
      ...
      expect(heroService.log).toHaveBeenCalledTimes(1);
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: deleted hero id=${this.mockHero.id}`
      );
    });
  ...
  describe('searchHero', () => {
    it('should find heroes matching the search criteria', () => {
      ...
      heroService
        ...
        .subscribe(
          (response) =>
            expect(response).toEqual([this.mockHeroes[1], this.mockHeroes[2]]),
          fail
        );

      // Receive PUT request
      const req = httpTestingController.expectOne(
        `${heroService.heroesUrl}/?name=${searchTerm}`
      );
      expect(req.request.method).toEqual('GET');
      // Respond with the updated hero
    ...
    it('should fail gracefully on error', () => {
      ...
      expect(heroService.messageService.messages[0]).toEqual(
        `HeroService: searchHeroes failed: Http failure response for ${heroService.heroesUrl}/?name=${searchTerm}: 404 Bad Request`
      );
    });
  });
